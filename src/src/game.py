    ...
    def updateScore(self, poseResults):
        ...
        for wristLocation in poseResults:
            ...
            for rectangle in self.rectList:
                ...
                y1 = rectangle[2]
                y2 = rectangle[3]
                if (x1 < wristLocation[1] < x2) and (y1 < wristLocation[0] < y2): # using the bounding box to find the intersection
                    self.rectList.remove(rectangle)
                    self.score += 5

                    
    def generateRect(self, currentTime, dt): # add the interval in which to add coords
        x1 = random.randint(30, 400)
        x2 = x1 + self.rectSize
        y1 = random.randint(30, 400)
        y2 = y1 + self.rectSize
        rectCoords = [x1, x2, y1, y2]
        
        currentTime = decimal.Decimal(str(currentTime))
        dt = decimal.Decimal(str(dt))
        result = currentTime % dt
        if result == 0:
